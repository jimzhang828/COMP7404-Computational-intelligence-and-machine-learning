...
class TicTacToeAgent():
    ...
    def getPattern(self, board):
        ...
        for board in boards:
            if tuple(board) in self.patterns:
                return self.patterns[tuple(board)]
        # debug
        # for board in boards:
        #     if tuple(board) not in self.patterns:
        #         print(tuple(board))
        # input('error...')
    ...
    def evaluation(self, gameState, gameRules):
        ...
        for board in gameState.boards:
            if not gameRules.deadTest(board):
                conf += self.getPattern(board)
        return ''.join(sorted(conf)) in self.winConf
    ...
    def getAction(self, gameState, gameRules):
        for action in gameState.getLegalActions(gameRules):
            ...
            if self.evaluation(succesor, gameRules):
                return action
        # input('error...')
        return random.choice(gameState.getLegalActions(gameRules))
